...
import type { ShopBranch } from "@/lib/types"

export function shopsRecord() {
  ...
  const fetchData = async (currentPage: number, pageSize: number, keyword = "") => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8088/finance-payment-confirmation/api/v1/branches?keyword=${keyword}&pageNumber=${currentPage}&pageSize=${pageSize}`,
        {
          headers: {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch shops record: ${response.status}`)
      }

      const result = await response.json()
      setShops(result.content) // store the array
      setTotalItems(result.pageable.totalElements)
    } catch (err) {
      console.error("Error fetching shops record:", err)
      //setError(err.message)
      toast({
        title: "Error",
        description: "Failed to load shops record",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  useEffect(() => {
    ...
    const handleRefresh = () => {
      toast({
        title: "Refreshing",
        description: "Updating shop records...",
      })
      fetchData(currentPage, pageSize, searchQuery)
    }
